...
void parent_login(fstream& reg_par_file);
void par_sta_menu();
void admin_login();
...
void weekly_menu_list(int menu_no)                     
{
    int numOfLines = 0, random;
    struct Menu menu;

    menu_file.open("menu_file.dat", ios::in | ios::binary);  // opening with reading and binary mode
...
void parent_reg(struct Parent_reg par_reg)
{
    ...
    reg_par_file.open("Reg_parent.dat", ios::out | ios::app | ios::binary);  // opening in writing, out and binary mode
...
void staff_reg(struct Staff_reg sta_reg)
{
    ...
    reg_sta_file.open("Reg_staff.dat", ios::out | ios::app | ios::binary);   // opening in writing, out and binary mode
...
void parent_login(fstream& reg_par_file)
{
    ...
    /*try:
        ...
        while (!reg_par_file.eof())  //file.peek() != EOF)
        {
            reg_par_file.read(reinterpret_cast<char*>(&par_reg), sizeof(par_reg));

            if((strcmp(username, par_reg.username) == 0) && (strcmp(pass, par_reg.password) == 0))
            {
                par_sta_menu();
                break;
            }
        ...
        reg_par_file.close();*/
            ...
            while (!reg_par_file.eof())  //file.peek() != EOF)
            {
                reg_par_file.read(reinterpret_cast<char*>(&par_reg), sizeof(par_reg));
                    ...
                    while((strcmp(username, par_reg.username) != 0) && (strcmp(pass, par_reg.password)) != 0 && i <= 2)
                    {
                        cout << "\nPlease check your password or username....try again!!\n";
                        cout << "\n\t\tEnter User Name : ";
                        cin.getline(username, 10);
                    ...
                    if((strcmp(username, par_reg.username) == 0) && (strcmp(pass, par_reg.password)) == 0 && i <= 3)
                    {
                        par_sta_menu();
                        break;
            ...
            reg_par_file.close();   //else//((strcmp(username, par_reg.username) != 0) && (strcmp(pass, par_reg.password) != 0))
...
void staff_login(char username[], char password[])
{
	...
	reg_sta_file.open("Reg_staff.dat", ios::in | ios::binary);  // opening in reading and binary mode

	while (!reg_sta_file.eof())  
	{
		reg_sta_file.read(reinterpret_cast<char*>(&sta_reg), sizeof(sta_reg));   // reading the file
	...
	{
		cout << "\n\n\t\t******************************************\n\n";
		cout << "\nSorry user not found .. Please register yourself using option one...";
		cout << "\n\n\t\t******************************************\n\n";
	}
	reg_sta_file.flush();
	reg_sta_file.close();
...
void par_sta_menu()
{
    ...
    do
    {
        ...
        case 1: 
            ...
            menu_file.open("menu_file.dat", ios::in | ios::binary);  // opening in reading and binary mode
...
void order_screen(fstream& order_file)
{
    ...
    if(order.quantity == 'l')
    {
        if(order.voucher == "yoobee")            // price after discounted  for l
        {
    ...
    {
        if(order.voucher == "yoobee")              // price after discounted for m
        {
    ...
    {
        if(order.voucher == "yoobee")              // price after discounted for s
        {
    ...
    order_file.open("Order_file.dat", ios::out | ios::app | ios::binary);     // opening in writing, out and binary mode

	  order_file.write(reinterpret_cast<char*>(&order), sizeof(order));         // wrting to order_file

	order_file.close(); 
    
    cout << "\n\n\nHere is your bill...\n";  // generating a bill after order
    ...
    do
    {
        ...
        case 1:
            ...
            do{
                ...
                menu_file.open("menu_file.dat", ios::out | ios::app | ios::binary);  // opening with writing, out and binary mode
        ...
        case 3:
            ...
            order_file.open("Order_file.dat", ios::in | ios::binary);  // opening with reading and binary mode
